        ...
        <nav>
            <ul className="">
                {props.props.map(( file: any ) => ( // checking by just the file.id is unsafe, but should be fine with the scale of the website
                    <li key={file.id} className={`hover:bg-brown hover:text-tan hover:transition-all ${(currentPath.match(file.id)) ? "pointer-events-none bg-brown text-tan" : "bg-yellowgray"}`}>
